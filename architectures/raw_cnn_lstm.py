...
class RawCNNLSTM(nn.Module):
    def __init__(self, model_params, metadata):
        ...
        lstm_hidden_size = model_params.get("lstm_hidden_size", model_params.get("gru_hidden_size", 32))
        ...
        for _ in range(num_blocks):
            layers.append(nn.Conv1d(block_in_channels, block_out_channels, kernel_size=7, padding=3))
            if use_batch_norm:
                layers.append(nn.BatchNorm1d(block_out_channels))
        ...
        self.proj = nn.Linear(block_in_channels, lstm_hidden_size)
        ...
        temporal_out_size = lstm_hidden_size * (2 if temporal_bidirectional else 1)
    ...
    def forward_sequence(self, windows, lengths=None):
        batch_size, num_windows, channels, sample_length = windows.shape
        windows = windows.reshape(batch_size * num_windows, channels, sample_length)
        ...
        embeddings = embeddings.reshape(batch_size, num_windows, -1)
        ...
        if lengths is not None:
            ...
            packed_out, _ = self.temporal_lstm(packed)
            ...
            temporal_out, _ = self.temporal_lstm(embeddings)
