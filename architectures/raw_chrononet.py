...
class Conv1dSame(nn.Module):
    ...
    def _pad_same(self, x):
        ...
        pad_len = max((out_len - 1) * stride + (kernel - 1) * dilation + 1 - in_len, 0)
...
class ChronoBlock1D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes=(2, 4, 8), stride=2):
        super().__init__()
        self.convs = nn.ModuleList(
            [Conv1dSame(in_channels, out_channels, kernel_size=ks, stride=stride) for ks in kernel_sizes]
        )
        self.relu = nn.ReLU()
...
class RawChronoNet(nn.Module):
    def __init__(self, model_params, metadata):
        ...
        self.bn = nn.BatchNorm1d(block_in_channels) if use_batch_norm else None
    ...
    def encode_window(self, x):
        ...
        return self.dropout(gru_out4[-1])
    ...
    def forward_sequence(self, windows, lengths=None):
        ...
        if self.truncate_bptt and self.truncate_bptt > 0:
            ...
            for start in range(0, num_windows, trunc):
                ...
                chunk_flat = chunk.reshape(batch_size * chunk_len, channels, sample_length)
                ...
                embeddings = embeddings.reshape(batch_size, chunk_len, -1)
